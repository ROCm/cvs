...
.. code:: bash
  ...
  cvs scp --file /path/to/local/file --cluster_file ~/cvs_workspace/cluster.json

You can also use the ``CLUSTER_FILE`` environment variable:

.. code:: bash

  CLUSTER_FILE=~/cvs_workspace/cluster.json cvs scp --file /path/to/local/file
...
.. code:: bash
  ...
  cvs scp --file /path/to/local/directory --recurse --cluster_file ~/cvs_workspace/cluster.json
...
.. code:: bash
  ...
  cvs scp --file ./large_file --parallel 50 --cluster_file ~/cvs_workspace/cluster.json

  # Use fewer parallel operations to reduce network load
  cvs scp --file ./sensitive_data --parallel 5 --cluster_file ~/cvs_workspace/cluster.json
...
.. code:: bash
  ...
  cvs scp --file /home/user/file.txt --cluster_file ~/cvs_workspace/cluster.json
...
.. code:: bash
  ...
  cvs scp --file ./file.txt --dest /tmp/remote_file.txt --cluster_file ~/cvs_workspace/cluster.json
...
.. code:: bash
  ...
  cvs scp --file ./setup.sh --dest /tmp/setup.sh --cluster_file ~/cvs_workspace/cluster.json
  cvs exec --cmd "chmod +x /tmp/setup.sh" --cluster_file ~/cvs_workspace/cluster.json

  # Copy configuration and set ownership
  cvs scp --file ./app.conf --dest /etc/app.conf --cluster_file ~/cvs_workspace/cluster.json
  cvs exec --cmd "sudo chown root:root /etc/app.conf" --cluster_file ~/cvs_workspace/cluster.json
...
.. code:: bash
  ...
  cvs scp --file ./myapp --dest /opt/myapp --recurse --cluster_file ~/cvs_workspace/cluster.json
...
.. code:: bash
  ...
  cvs scp --file myapp.tar.gz --dest /tmp/myapp.tar.gz --cluster_file ~/cvs_workspace/cluster.json
  cvs exec --cmd "cd /opt && sudo tar xzf /tmp/myapp.tar.gz" --cluster_file ~/cvs_workspace/cluster.json

2. Reducing parallelism to avoid overwhelming the network:

.. code:: bash

  cvs scp --file ./large_directory --dest /opt/data --recurse --parallel 5 --cluster_file ~/cvs_workspace/cluster.json
...
.. code:: bash
  ...
  cvs scp --file ~/cvs/cvs/input/env_file/rccl/ainic_env_script.sh --dest /tmp/ainic_env_script.sh --cluster_file ~/cvs_workspace/cluster.json
  ...
  cvs scp --file openmpi.tar.gz --dest /tmp/openmpi.tar.gz --cluster_file ~/cvs_workspace/cluster.json
  cvs scp --file rccl-tests.tar.gz --dest /tmp/rccl-tests.tar.gz --cluster_file ~/cvs_workspace/cluster.json

  # Extract archives on all nodes
  cvs exec --cmd "sudo tar xzf /tmp/openmpi.tar.gz -C /opt" --cluster_file ~/cvs_workspace/cluster.json
  cvs exec --cmd "sudo tar xzf /tmp/rccl-tests.tar.gz -C /opt" --cluster_file ~/cvs_workspace/cluster.json
