...
.. code:: text
  ...
  --------------------------------------------------------------------------------
    ...
    cvs.tests.inference.xdit (5 test suites)
  ...
  ================================================================================
  Total: 32 test suites across 1 package(s)
...
- ``--workspace``: The shared-filesystem root for this run's artifacts. The run directory becomes ``<workspace>/cvs_runs/<run_id>`` and is available to test configs as the ``{run_dir}`` placeholder. If omitted, it falls back to ``$CVS_WORKSPACE`` and then to the venv's parent directory. In a scheduler-managed run (Slurm or SPUR) the container orchestrator bind-mounts the run directory into the container at the same path, so ``{run_dir}`` resolves identically on the host and inside the container.
...
PyTorch XDit suites live under ``cvs.tests.inference.xdit``. Config templates are in
