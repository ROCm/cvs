...
- ``--workspace``: The shared-filesystem root for this run's artifacts. The run directory becomes ``<workspace>/cvs_runs/<run_id>`` and is available to test configs as the ``{run_dir}`` placeholder. If omitted, it falls back to ``$CVS_WORKSPACE`` and then to the venv's parent directory.
