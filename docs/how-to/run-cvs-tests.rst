...
- ``-s``: The script that will be executed by Pytest.
- ``--cluster_file``: The location of the cluster file which has the details of the cluster, IPs, and access details.
- ``--config_file``:- This is the configuration file used for the test. Depending on the test suite being run, the configuration may vary. The sample input files are organized as sub-directories under the ``cvs/input`` folder similar to the Pytests.
...
You can also create a wrapper shell script to run multiple test suites sequentially by putting the different Pytest run commands in a bash script as described in the README under the ``cvs/tests/health`` folder.
...
The host check scripts, which can validate anything on the host side like the new model load balancing enable, PCIE checks, kernel version, ROCm version, and so on.
...
The output of the test is captured in an HTML report file generated by Pytest. 
It has summary of the number of test cases that have passed/failed. You can navigate to the logs directly in the browser from this report.

CVS creates a log folder and captures the output of every test in a log file here: ``$HOME/tmp/test.log``

.. tip::

  If there are any errors which prevent the tests from starting, an error message with an appropriate suggestion will be printed on the console.

Test output examples
--------------------

Test results are also available on the console.
