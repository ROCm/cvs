...
.. note::
  ...
  - Host-mount server wrappers live under ``cvs/input/.../<variant>/benchmark_server_scripts/`` (checkout / editable). Set ``host_benchmark_scripts_relpath`` in the suite config to that directory **relative to the ``cvs`` package root** (default matches MI300X GPT-OSS 120B single). With ``benchmark_server_script_path`` ``auto``, CVS reads the entry script from the driver checkout when present, otherwise from ``cvs.lib.inference.inferencemax_host_scripts.BUNDLED_SERVER_SCRIPTS`` (keyed by the ``server_script`` basename after ``fixed_seq_len/`` stripping), then writes those bytes under ``{log_dir}/inference-max/host_scripts_staged/<variant>/`` on **each** GPU node using ``docker exec`` into the inference container (root in-container), so the path exists on the bind-mounted home even when pytest runs elsewhere and when ``log_dir`` subtrees were created earlier by Docker and are not writable over SSH as the login user. Register new models in ``BUNDLED_SERVER_SCRIPTS`` when needed. Set an explicit absolute ``benchmark_server_script_path`` to skip deploy and use that directory directly on the nodes.
