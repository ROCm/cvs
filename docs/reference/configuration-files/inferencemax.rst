...
.. note::
  ...
  - Host-mount server wrappers for GPT-OSS MI300X ship under ``cvs/input/.../benchmark_server_scripts/`` (checkout and editable installs). The shipped ``mi300x_gpt_oss_120b_single_config.json`` sets ``benchmark_server_script_path`` to ``auto``: ``InferenceMaxJob`` uses that directory when present; otherwise it writes the same bundled wrapper (string in ``cvs.lib.inference.inferencemax_orch``) to ``{log_dir}/inference-max/host_scripts_bundled/`` (under your home mount so the container can ``cd`` there). Set an explicit absolute ``benchmark_server_script_path`` if you prefer a fixed path.
