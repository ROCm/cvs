...
.. note::
  ...
  - Host-mount server wrappers live under ``cvs/input/.../<variant>/benchmark_server_scripts/`` (checkout / editable). Set ``host_benchmark_scripts_relpath`` in the suite config to that directory **relative to the ``cvs`` package root** (default matches MI300X GPT-OSS 120B single). With ``benchmark_server_script_path`` ``auto``, CVS **always** copies or materializes the entry script under ``{log_dir}/inference-max/host_scripts_staged/<variant>/`` (under the mounted home) so ``docker exec`` never changes directory into a ``site-packages`` path that exists only on the pytest host. The script comes from checkout when found, otherwise from ``cvs.lib.inference.inferencemax_host_scripts.BUNDLED_SERVER_SCRIPTS`` (keyed by the ``server_script`` basename after ``fixed_seq_len/`` stripping). Register new models there when needed. Set an explicit absolute ``benchmark_server_script_path`` to skip staging and use that directory directly.
...
.. list-table::
   ...
   * - ``benchmark_server_script_path``
     ...
     - With ``auto``, CVS copies or materializes the server script under ``{log_dir}/inference-max/host_scripts_staged/`` (from checkout when present, else ``inferencemax_host_scripts``). An absolute path skips staging and uses that directory (must be visible inside the container via ``volume_dict``).
   * - ``host_benchmark_scripts_relpath``
     - MI300X default: ``input/config_file/inference/inferencemax_single/mi300x_gpt_oss_120b_single/benchmark_server_scripts``
     - Path to the variant's ``benchmark_server_scripts`` directory, **relative to the ``cvs`` package root**, used when ``benchmark_server_script_path`` is ``auto``. Point each new InferenceMax variant JSON at its own ``.../<variant>/benchmark_server_scripts`` tree.
   ...
   * - ``benchmark_params.`` |br| ``gpt-oss-120b.server_script``
     ...
     - Path under ``benchmarks/<single_node|multi_node>/`` to the shell script inside the clone (must exist in ``inferencemax_repo``). With ``use_host_mounted_server_script`` and ``benchmark_server_script_path`` ``auto``, CVS maps ``fixed_seq_len/`` to the flat script name and runs it from ``host_scripts_staged`` under ``log_dir`` (content from checkout or bundled); with an explicit ``benchmark_server_script_path``, it uses that host directory.
