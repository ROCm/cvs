...
.. note::
  ...
  - Host-mount server wrappers live under ``cvs/input/.../<variant>/benchmark_server_scripts/`` (checkout / editable). Set ``host_benchmark_scripts_relpath`` in the suite config to that directory **relative to the ``cvs`` package root** (default matches MI300X GPT-OSS 120B single). With ``benchmark_server_script_path`` ``auto``, CVS reads the entry script from the driver checkout when present, otherwise from ``cvs.lib.inference.inferencemax_host_scripts.BUNDLED_SERVER_SCRIPTS`` (keyed by the ``server_script`` basename after ``fixed_seq_len/`` stripping), then **deploys** those bytes over cluster SSH into ``{log_dir}/inference-max/host_scripts_staged/<variant>/`` on **each** GPU node before ``docker exec``, so the path exists under the bind-mounted home even when pytest runs on a different machine than the GPUs. Register new models in ``BUNDLED_SERVER_SCRIPTS`` when needed. Set an explicit absolute ``benchmark_server_script_path`` to skip deploy/staging and use that directory directly on the nodes.
...
.. list-table::
   ...
   * - ``benchmark_server_script_path``
     ...
     - With ``auto``, CVS deploys the server script bytes (from driver checkout when present, else ``inferencemax_host_scripts``) onto each node under ``{log_dir}/inference-max/host_scripts_staged/`` before launch. An absolute path skips deploy and uses that directory on the nodes (must be visible inside the container via ``volume_dict``).
