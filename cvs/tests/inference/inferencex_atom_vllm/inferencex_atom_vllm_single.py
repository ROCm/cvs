...
InferenceX vLLM (ROCm) parity suite — same workload cards as ``inferencex_atom_single``
with ``params.driver=vllm`` (``vllm serve`` + ``vllm bench serve``).
'''
...
_spec = _ilu.spec_from_file_location("_inferencex_atom_vllm_shared", _pl.Path(__file__).with_name("_shared.py"))
...
def _log_variant_run_card(variant_config):
    ...
    log.info("InferenceX vLLM parity run card: %s", "; ".join(parts))
...
def test_inferencex_atom_vllm_inference(
    ...
    job = InferenceXAtomJob(
        ...
        log_subdir="inferencex-atom-vllm",
