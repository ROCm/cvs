...
class TestAortaBenchmark:
    ...
    def test_parse_results(self, aorta_runner_config, validated_aorta_config):
        """Parse benchmark results into metrics using Aorta's analysis reports."""
        globals.error_list = []
        ...
        # Use AortaReportParser if tracelens_analysis artifact exists
        # This parses the Excel reports generated by Aorta's scripts
        has_tracelens_analysis = run_result.get_artifact("tracelens_analysis") is not None or (
            run_result.get_artifact("torch_traces")
            and (run_result.get_artifact("torch_traces").parent / "tracelens_analysis").exists()
        )

        if has_tracelens_analysis:
            log.info("Using AortaReportParser (parsing Aorta's Excel reports)")
            parser = AortaReportParser()
        else:
            log.info("Falling back to TraceLensParser (parsing raw traces)")
            parser = TraceLensParser(use_tracelens=True)

        # Parse results
        parse_result = parser.parse(run_result)
    ...
    def test_validate_thresholds(self, validated_aorta_config):
        ...
        # Use AortaReportParser for validation (same interface as TraceLensParser)
        parser = AortaReportParser()
