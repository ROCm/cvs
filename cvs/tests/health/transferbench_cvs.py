...
def parse_tb_a2a_bw(out_dict, exp_dict):
    for node in out_dict.keys():
        ...
        rtotal_list = re.findall(
            r'│\s+RTotal\s+│\s+([0-9\.]+)\s+([0-9\.]+)\s+([0-9\.]+)\s+([0-9\.]+)\s+([0-9\.]+)\s+([0-9\.]+)\s+([0-9\.]+)\s+([0-9\.]+)',
            out_dict[node],
        )
        if not rtotal_list:
...
def parse_tb_example_test_results(out_dict, exp_dict):
    for node in out_dict.keys():
        ...
        match = re.search(r'Executor:\s+GPU\s+[0-9]+\s+│\s*([0-9\.]+)\s+GB/s', test1_out, re.I)
        if not match:
        ...
        match = re.search(r'Executor:\s+(GPU|DMA)\s+[0-9]+\s+│\s*([0-9\.]+)\s+GB/s', test2_out, re.I)
        if not match:
            fail_test(f"Transfer bandwidth pattern not found in Test 2 output on node {node}. Output: {test2_out}")
            continue
        test2_res = match.group(2)
        ...
        match = re.search(r'Executor:\s+GPU\s+[0-9]+\s+│\s*([0-9\.]+)\s+GB/s', test4_out, re.I)
        if not match:
