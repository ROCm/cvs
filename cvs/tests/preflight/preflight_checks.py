...
def phdl(cluster_dict, config_dict):
    ...
    # Create config with parallel_group_size from preflight config
    hosts_per_shard = get_nested_config(config_dict, 'parallelism', 'parallel_group_size', 32)
...
def test_rdma_connectivity(phdl, cluster_dict, config_dict):
    ...
    port_range = get_nested_config(config_dict, 'connectivity_check.rdma', 'ibv_test_port_range', '9000-9999')
    timeout = int(get_nested_config(config_dict, 'connectivity_check.rdma', 'ibv_test_timeout', '10'))
    expected_interfaces = get_nested_config(
        config_dict, 'node_check', 'rdma_interfaces', ["rocep28s0", "rocep62s0", "rocep79s0", "rocep96s0"]
    )
    gid_index = get_nested_config(config_dict, 'node_check', 'gid_index', '3')
    parallel_group_size = get_nested_config(config_dict, 'parallelism', 'parallel_group_size', 128)
