...
def config_file(pytestconfig):
    ...
    For single-node TransferBench testing, use:
...
def test_transferbench_perf(phdl, shdl, cluster_dict, config_dict, transfer_config, num_bytes ):
    ...
    """
    Execute TransferBench performance test on a single node with given parameters.
    ...
    Parameters (from fixtures and config):
      ...
      - transfer_config: TransferBench transfer configuration string (e.g., "1 4 (G0->G0->G0)").
    ...
    Flow:
      ...
      3) Invoke transferbench_lib.transferbench_cluster_test with single-node parameters.
    ...
    globals.error_list = []
    node_list = list(cluster_dict['node_dict'].keys())[:1]  # Only use first node for single-node test

    # Build list of nodes and their VPC IPs (only first node)
    vpc_node_list = []
    ...
    # Execute the TransferBench cluster test with single-node parameters
