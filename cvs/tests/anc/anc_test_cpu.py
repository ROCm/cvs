...
'''
ANC CPU validation suite.

Pre-task: install ANC (dispatched by release-archive flavour) at the target
anc_version. Core task: run the full CPU group set in a single
...
Per-node artifacts are downloaded to:
    <runner_log_folder>/anc/<ip>_<hostname>/test_cpu/
...
class TestAncCpuPreTasks:
    ...
    def test_install_anc(self, phdl, config_dict):
        ...
        '''
        log.info("ANC CPU Pre-Task: install/verify ANC")
    ...
    def test_rocm_ldconfig(self, phdl):
        ...
        genuinely missing.
        '''
        log.info("ANC CPU Pre-Task: ensure ROCm ldconfig")
        anc_lib.ensure_rocm_ldconfig(phdl)


class TestAncCpuCoreTasks:
    '''Core ANC CPU validation.'''

    def test_cpu(self, phdl, cluster_dict, config_dict):
        '''
        Run the ANC CPU validation groups on all nodes.

        Executes ``sudo ./anc.py -g <CPU_GROUPS>`` under the ANC install dir
        ...
        '''
        log.info("ANC CPU Core Task: groups=%s", anc_lib.CPU_GROUPS)
        anc_lib.run_anc_groups(
            phdl, cluster_dict, config_dict, anc_lib.CPU_GROUPS, "test_cpu"
        )


class TestAncCpuPostTasks:
