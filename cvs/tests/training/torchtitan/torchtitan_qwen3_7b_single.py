...
def test_qwen3_7b_single_node(phdl, gpu_type, training_dict, model_params_dict, hf_token):
    """
    Pytest: Single-node TorchTitan Qwen3 7B training lifecycle test.

    Qwen3 is a hybrid model architecture supported by TorchTitan with attention
    scaling for variable-length sequences. Single-node training uses all 8 GPUs on one node.

    Args:
      phdl: Cluster handle used by the training job to execute commands.
      gpu_type (str): GPU identifier (e.g., 'mi350', 'mi355', 'mi300x') used for selecting params.
    ...
    Behavior:
      ...
      - Instantiates TorchTitanTrainingJob with Qwen3-7B model.
