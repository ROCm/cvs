...
def gpu_type(
    ...
    Returns:
      str: The GPU type (e.g., 'mi350', 'mi355') used to select model parameters and logic.
...
def test_llama_3_1_70b_single_node(phdl, gpu_type, training_dict, model_params_dict, hf_token):
    """
    Pytest: Single-node TorchTitan Llama 3.1 70B training lifecycle test.

    Args:
      phdl: Cluster handle used by the training job to execute commands.
      gpu_type (str): GPU identifier (e.g., 'mi350', 'mi355') used for selecting params.
    ...
    Behavior:
      ...
      - Instantiates TorchTitanLlamaTrainingJob with tuned parameters disabled.
