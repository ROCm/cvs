...
def gpu_type(
    ...
    Returns:
      str: The GPU type (e.g., 'mi300x', 'mi325') used to select model parameters and logic.
...
def test_deepseek_v3_16b_distributed(phdl, gpu_type, training_dict, model_params_dict, hf_token):
    """
    Pytest: Multi-node TorchTitan DeepSeek-V3 16B training lifecycle test.

    DeepSeek-V3 is a Mixture of Experts (MoE) model. Multi-node distributed
    training leverages expert parallelism across nodes for optimal performance.

    Args:
      phdl: Cluster handle used by the training job to execute commands.
      gpu_type (str): GPU identifier (e.g., 'mi300x', 'mi325') used for selecting params.
    ...
    Behavior:
      ...
      - Instantiates TorchTitanTrainingJob with distributed training enabled.
