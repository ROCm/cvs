...
from cvs.lib import torchtitan_training_lib
...
def gpu_type(
    ...
    Returns:
      str: The GPU type (e.g., 'mi350', 'mi355', 'mi300x') used to select model parameters and logic.
...
def test_launch_torchtitan_containers(phdl, training_dict):
    """
    Pytest: Launch TorchTitan training containers and verify launch step.
    ...
    Args:
      ...
      training_dict (dict): Training configuration including:
        ...
        - 'container_image': Docker image to use (e.g., rocm/primus:v26.2)
        - 'container_config': {
    ...
    log.info('Testcase launch TorchTitan containers')
    globals.error_list = []
    container_name = training_dict['container_name']
...
def test_deepseek_v3_16b_single_node(phdl, gpu_type, training_dict, model_params_dict, hf_token):
    """
    Pytest: Single-node TorchTitan DeepSeek-V3 16B training lifecycle test.

    DeepSeek-V3 is a Mixture of Experts (MoE) model with fine-grained experts
    and shared-expert isolation. Single-node training uses all 8 GPUs on one node.

    Args:
      phdl: Cluster handle used by the training job to execute commands.
      gpu_type (str): GPU identifier (e.g., 'mi300x', 'mi325', 'mi350', 'mi355') used for selecting params.
    ...
    Behavior:
      ...
      - Instantiates TorchTitanTrainingJob with DeepSeek-V3 model.
