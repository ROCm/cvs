...
- **OrchestratorFactory** (`cvs/core/orchestrators/factory.py`) instantiates a
  concrete orchestrator from `OrchestratorConfig.from_configs(cluster_file,
  test_config)`.
...
2. **Register in the factory.** Edit
   ...
   def create(runtime_name, log, orchestrator):
       ...
       elif runtime_name == 'podman':                  # NEW
           return PodmanRuntime(log, orchestrator)     # NEW
       else:
...
3. **Register in the factory.** Edit
   ...
   elif orchestrator_type == 'slurm':                              # NEW
       return SlurmOrchestrator(log, config, stop_on_errors)       # NEW
   else:
