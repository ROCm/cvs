...
SSH Key Scan Commands:
  ...
  CLUSTER_FILE=cluster.json cvs sshkeyscan --at head           Use env var for cluster file"""
    ...
    def scan_host_key_remote(self, pssh_handle, host, known_hosts_file, remove_existing=False, dry_run=False):
        """Scan SSH host key for a single host (remote execution via pssh)."""
        try:
            # Remove existing key if requested
            if remove_existing and not dry_run:
                remove_cmd = f"ssh-keygen -f {known_hosts_file} -R {host}"
                pssh_handle.exec(remove_cmd, print_console=False)
            ...
            result_dict = pssh_handle.exec(scan_cmd, timeout=35, print_console=False)

            # Get result for the head node (should be only one host in pssh_handle)
            ...
            append_result = pssh_handle.exec(append_cmd, print_console=False)
    ...
    def run(self, args):
        ...
        if args.at == 'head':
            ...
            pssh_handle = Pssh(None, [head_node], user=username, pkey=priv_key_file, env_vars=env_vars)
        ...
        if args.dry_run:
            ...
            if args.at == 'local':
                ...
                pssh_handle.exec(setup_cmd, print_console=False)
