  ...
  "preflight": {
    ...
    "connectivity_check": {
      "rdma": {
        ...
        "ibv_test_timeout": "10",
        "ibv_test_port_range": "9000-9999",
        "inter_group_wave_pairs": "auto"
      }
    },
    "reporting": {
...
```
preflight/
├── parallelism/          # Parallel execution settings
...
│   └── rdma/            # RDMA-specific parameters
│   └── ifoe/            # (Future: IFoE parameters)
└── reporting/           # Output and report generation
```

### Execution Flow
1. **parallelism** - Configure parallel execution resources
2. **node_check** - Validate individual nodes in parallel
...
- **`parallel_group_size`** (default: 128)
...
- **`ibv_test_port_range`** (default: "9000-9999")
...
- **`inter_group_wave_pairs`** (default: "auto")
...
- **Small parallel_group_size (16-32)**: More rounds, less resource usage per node, better for resource-constrained environments
- **Large parallel_group_size (128+)**: Fewer rounds, more resource usage per node, faster overall completion
- **Debug mode impact**: `scriptlet=true` adds 10-30% overhead due to strace logging

**Advanced Parameter Tuning:**
- **inter_group_wave_pairs**: Lower values (2-4) reduce memory/CPU load but increase test time
