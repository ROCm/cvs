...
from app.core.go_collector import _exec_one, query_daemon_health, _refresh_nodes_in_daemon
...
class SshManager:
        ...
        try:
            if self.jump_host:
                channel = await asyncio.to_thread(
                    self._open_jump_channel, remote_port
                )
            else:
                channel = await asyncio.to_thread(
                    self._open_direct_channel, node, remote_port
                )
        except Exception:
            asyncio_end.close()
            thread_end.close()
    ...
    def _ensure_jump_connection(self) -> None:
        ...
        with self._jump_lock:
            transport = (
                self._jump_client.get_transport() if self._jump_client else None
            )
