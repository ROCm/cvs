...
class TraceLensParser:
    ...
    def parse_trace_directory(self, trace_dir: Path) -> ParseResult[AortaTraceMetrics]:
        ...
        if not trace_files:
            return ParseResult(status=ParseStatus.FAILED, errors=[f"No trace files found in {trace_dir}"])
    ...
    def _parse_with_tracelens(self, trace_file: Path, rank: int) -> AortaTraceMetrics:
        ...
        try:
            ...
            if 'gpu_timeline' in result_dfs:
                ...
                log.info(
                    f"  Total comm: {total_comm_ms:.2f}ms (overlap: {100 * (total_comm_ms - exposed_comm_ms) / total_comm_ms:.1f}%)"
                )

            if total_time_us > 0:
                log.info(
                    f"TraceLens metrics for rank {rank}: total={total_time_us:.2f}us, compute={compute_time_us:.2f}us, comm={comm_time_us:.2f}us"
                )
                ...
                log.warning("TraceLens returned no usable metrics, falling back to basic parsing")
    ...
    def aggregate(
        self, parse_result: ParseResult[AortaTraceMetrics], num_nodes: int, gpus_per_node: int, **metadata
        ...
        try:
            return AortaBenchmarkResult.from_rank_metrics(
                metrics=parse_result.results, num_nodes=num_nodes, gpus_per_node=gpus_per_node, **metadata
            )
        except Exception as e:
            log.exception(f"Aggregation failed: {e}")
            return None

    def validate_thresholds(self, result: AortaBenchmarkResult, expected: Dict[str, Any]) -> List[str]:
        """
        ...
        if min_compute_ratio is not None:
            if result.avg_compute_ratio < min_compute_ratio:
                failures.append(
                    f"Average compute ratio {result.avg_compute_ratio:.2%} below threshold {min_compute_ratio:.2%}"
                )
        ...
        if min_overlap_ratio is not None:
            if result.avg_overlap_ratio < min_overlap_ratio:
                failures.append(
                    f"Average overlap ratio {result.avg_overlap_ratio:.2%} below threshold {min_overlap_ratio:.2%}"
                )
        ...
        if max_time_variance is not None:
            if result.avg_iteration_time_us > 0:
                ...
                if variance_ratio > max_time_variance:
                    failures.append(
                        f"Iteration time variance {variance_ratio:.2%} exceeds threshold {max_time_variance:.2%}"
                    )
