...
class AortaReportParser:
    ...
    def parse_analysis_directory(self, analysis_dir: Path) -> ParseResult[AortaTraceMetrics]:
        ...
        if not individual_dir.exists():
            return ParseResult(status=ParseStatus.FAILED, errors=[f"Individual reports not found: {individual_dir}"])
        ...
        if not report_files:
            ...
            has_profiler_structure = (
                torch_profiler_dir.exists()
                and any(d.is_dir() and d.name.startswith("rank") for d in torch_profiler_dir.iterdir())
            )
    ...
    def _parse_individual_report(self, report_file: Path) -> AortaTraceMetrics:
        ...
        _total_comm_ms = timeline.get('total_comm_time', 0)  # noqa: F841 - kept for future use
        ...
        log.debug(
            f"Rank {rank}: total={total_time_ms:.1f}ms, "
