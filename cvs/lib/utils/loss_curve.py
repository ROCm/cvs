...
Loss-curve PNG rendering shared by the training suites.

Kept separate from the suites' log/artifact parsers because it does file I/O
and lazily imports matplotlib. matplotlib is imported inside the function with
the headless ``Agg`` backend so that importing this module never hard-requires
the dependency, and a missing/broken matplotlib degrades to ``None`` rather
than failing the run -- the loss-curve verdict is computed independently of the
plot.
'''
