...
def resolve_distributed_execution_hosts(cluster_dict, inference_dict):
    """First ``nnodes`` hosts from ``cluster.json``; rank-0 is rendezvous and benchmark.

    Ignores ``server_node_list`` and ``benchmark_serv_node``. Fails if ``nnodes``
    is less than 2 or larger than the cluster.
    """
    hosts = resolve_server_nodes(cluster_dict, inference_dict)
    nnodes = resolve_nnodes(inference_dict, hosts)
    if nnodes < 2:
        raise ValueError(f"distributed xDiT requires nnodes >= 2, got {nnodes}")
    if len(hosts) < nnodes:
        raise ValueError(f"distributed xDiT requests {nnodes} nodes but cluster.json only has {len(hosts)}")
    return hosts[:nnodes]
