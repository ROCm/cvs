...
class TestXditConfigLoader(unittest.TestCase):
    ...
    def _distributed_payload(self, nnodes=2):
        return {
            "schema_version": 1,
            "framework": "xdit",
            "gpu_arch": "mi300x",
    ...
    def test_distributed_selects_first_nnodes_from_cluster(self):
        path = self._write_config(self._distributed_payload(nnodes=2), {"auto": {"max_avg_pipe_time_s": 300.0}})
        cluster = {"node_dict": {"node-a": {}, "node-b": {}, "node-c": {}}}

        variant = load_variant(path, cluster)

        self.assertEqual(variant.inference["_execution_hosts"], ["node-a", "node-b"])

    def test_distributed_fails_when_nnodes_less_than_2(self):
        path = self._write_config(
            self._distributed_payload(nnodes=1),
            {"auto": {"max_avg_pipe_time_s": 300.0}},
        )
    ...
    def test_distributed_fails_when_cluster_is_smaller_than_nnodes(self):
        path = self._write_config(
            self._distributed_payload(nnodes=2),
            {"auto": {"max_avg_pipe_time_s": 300.0}},
        )
        with self.assertRaisesRegex(ValueError, "requests 2 nodes"):
            load_variant(path, {"node_dict": {"node-a": {}}})
