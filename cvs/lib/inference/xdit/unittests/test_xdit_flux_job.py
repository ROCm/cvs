...
class TestFluxParallelism(unittest.TestCase):
    ...
    def test_distributed_hosts_take_first_nnodes(self):
        cluster = {"node_dict": {"10.0.0.1": {}, "10.0.0.2": {}, "10.0.0.3": {}}}
        hosts = resolve_distributed_execution_hosts(cluster, {"nnodes": 2, "server_node_list": ["10.0.0.3"]})
        self.assertEqual(hosts, ["10.0.0.1", "10.0.0.2"])

    def test_distributed_hosts_reject_nnodes_less_than_2(self):
    ...
    def test_distributed_hosts_reject_nnodes_larger_than_cluster(self):
        ...
        with self.assertRaisesRegex(ValueError, "requests 2 nodes"):
            resolve_distributed_execution_hosts(cluster, {"nnodes": 2})
...
class TestExecCmdListOnNodes(unittest.TestCase):
    @patch("cvs.lib.inference.xdit.xdit_flux_job._exec_on_single_node")
...
class TestPhdlConnectionKwargs(unittest.TestCase):
    def test_reads_credentials_from_inner_pssh(self):
        from cvs.lib.inference.xdit.xdit_flux_job import _phdl_connection_kwargs
