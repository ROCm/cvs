...
Unit tests for AtomJob.parse_results (stock ``results`` artifact -> client.*).
...
class TestATOMAtomOrchParse(unittest.TestCase):
...
class TestATOMAtomBuildServerCmd(unittest.TestCase):
    @staticmethod
...
class TestATOMAtomIsReady(unittest.TestCase):
