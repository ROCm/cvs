...
class PreflightReportGenerator(PreflightCheck):
    ...
    def run(self):
        ...
        if _config_flag_enabled(get_nested_config(self.config_dict, 'reporting', 'generate_html_report', 'true')):
    ...
    def _generate_html_report(self, output_path=None):
        ...
        if output_path is None:
            ...
            output_dir = get_nested_config(self.config_dict, 'reporting', 'report_output_dir', '/tmp/preflight_reports')
        ...
        rdma_csv_path = None
        if _config_flag_enabled(get_nested_config(self.config_dict, 'reporting', 'generate_rdma_pairs_csv', 'true')):
    ...
    def _generate_rdma_topology_html(self, connectivity_results):
        ...
        if ng >= 2 and mode == 'multi_wave':
            ...
            blocks.append(
                ...
                f'<code>inter_group_wave_pairs</code> (resolved chunk size <strong>{chunk}</strong>; '
    ...
    def _generate_pruned_nodes_after_intra_html(self, records):
        ...
        <div class="pruned-nodes" style="margin:15px 0;padding:12px;background:#fff3cd;border:1px solid #ffc107;border-radius:4px;">
            ...
            <p style="margin-top:0;">These nodes met the configured <strong>intra-group peer failure fraction</strong> threshold (see <code>prune_failure_threshold</code>): enough distinct peers in the same partition group had at least one failing intra test versus that node. They were omitted from inter-group tests.</p>
            <table class="summary-table">
