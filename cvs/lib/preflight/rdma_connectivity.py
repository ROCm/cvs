...
def get_nested_config(config_dict, section, key, default):
    ...
    Args:
        ...
        section: Section name (e.g., 'debug', 'connectivity_check.rdma')
...
class RdmaConnectivityCheck(PreflightCheck):
    ...
    def _scriptlet_enabled(self) -> bool:
        ...
        if not self.config_dict:
            return False
        v = get_nested_config(self.config_dict, 'debug', 'scriptlet', False)
        if isinstance(v, str):
    ...
    def _port_listen_retry_max(self) -> int:
        ...
        raw = get_nested_config(cfg, 'connectivity_check.rdma', 'port_retry_max', 3)
        try:
            n = int(raw)
    ...
    def _port_listen_retry_gap(self) -> int:
        ...
        cfg = self.config_dict or {}
        raw = get_nested_config(cfg, 'connectivity_check.rdma', 'port_retry_gap', 1000)
        try:
            g = int(raw)
        except (TypeError, ValueError):
            g = 1000
    ...
    @staticmethod
    def _generate_test_pair_key(assignment):
        """Stable result key for one directed iface test."""
        return f"{assignment['server_node']} <-> {assignment['client_node']} ({assignment['server_iface']}->{assignment['client_iface']})"

    @staticmethod
    def _is_port_listen_failure(test_result: dict) -> bool:
        if not isinstance(test_result, dict) or test_result.get('status') != 'FAIL':
            return False
        for line in test_result.get('error_details') or []:
    ...
    def _remote_rdma_workspace_root(self) -> str:
        ...
        Lives under ``artifacts_root_dir`` so operators can point NFS-mounted preflight reports here.
        """
        return f'{self._artifacts_root_dir()}/rdma_connectivity_workspace'

    def _begin_full_mesh_rdma_artifacts(self) -> None:
        """
        Once per ``full_mesh`` run: ``rm -rf`` remote workspace and create a new session id.

        Clears only ``<artifacts_root_dir>/rdma_connectivity_workspace``, not sibling HTML files.
        """
    ...
    def __init__(
        ...
        Args:
            ...
            mode: "basic", "full_mesh", or "skip"
            port_range: Port range for ibv_rc_pingpong tests (e.g., "10000-50000")
    ...
    def run(self):
        ...
        if self.mode == "full_mesh":
            ...
            self._process_test_results(all_results, results)

        else:
            # Basic mode using ScriptLet execution
            batch_results = self._execute_basic(port_start)
            self._process_test_results(batch_results, results)

        return results

    def _generate_interface_test_pairs(self):
        """
    ...
    def _process_test_results(self, test_results, results_container):
        """Process test results: count successes/failures and update node statistics."""
        for pair_key, pair_result in test_results.items():
            results_container['pair_results'][pair_key] = pair_result

            if pair_result['status'] == 'PASS':
                results_container['successful_pairs'] += 1
            else:
                results_container['failed_pairs'] += 1
            ...
            results_container['node_status'][server_node]['server_tests'] += 1
            results_container['node_status'][client_node]['client_tests'] += 1

            if pair_result['status'] == 'PASS':
                results_container['node_status'][server_node]['successful_tests'] += 1
                results_container['node_status'][client_node]['successful_tests'] += 1
            else:
                results_container['node_status'][server_node]['failed_tests'] += 1
                results_container['node_status'][client_node]['failed_tests'] += 1

        results_container['total_pairs'] = len(test_results)

    def _execute_basic(self, port_start):
        """
        Execute basic mode connectivity testing.
        ...
        Returns:
            dict: Test results
        """
        log.info(f"Starting {self.mode} mode connectivity testing")

        # Generate node pairs based on mode
        pairs = self._generate_interface_test_pairs()
        if not pairs:
            return {}

        # Convert pairs to assignments
        assignments = self._create_test_assignments(pairs, port_start)

        # Execute via ScriptLet infrastructure
        log.info(f"Executing {len(assignments)} test assignments via ScriptLet")
        results = self._execute_test_assignments(assignments, f"{self.mode}_mode", f"{self.mode}_single")

        log.info(f"Completed {self.mode} mode testing: {len(results)} results")
        return results

    def _create_test_assignments(self, pairs, base_port):
        """
        Convert node pairs to ScriptLet assignment format.

        Args:
            pairs: List of (server_node, client_node) tuples
            base_port: Starting port number for assignments
        ...
        # Convert each pair to bidirectional interface matrix assignments
        for node_a, node_b in pairs:
            # Test both directions: A->B and B->A (like full mesh)
            for server_node, client_node in [(node_a, node_b), (node_b, node_a)]:
    ...
    def _execute_intergroup_waves(self, groups_inter, port_start, cfg):
        ...
        Config (``cfg``):
            ...
            inter_full_mesh_group_pairs_per_wave: max ordered group-pairs (Gi->Gj keys)
                per wave when multi_wave. If omitted, empty, or ``auto``, uses **max(1, Ng−1)**
                where Ng is the number of inter-group partition groups.
            (Backward compatible aliases: ``inter_group_wave_pairs``.)

        Returns:
            tuple: (merged_results_dict, meta_dict with keys ``mode``, ``waves``)
        """
        full = self._generate_all_intergroup_pairs(groups_inter)
        if not full:
        ...
        if mode == 'single_shot':
            ...
            wave_res = self._execute_test_round_with_retries(
        ...
        raw = get_nested_config(
            cfg,
            'connectivity_check.rdma',
            'inter_full_mesh_group_pairs_per_wave',
            get_nested_config(cfg, 'connectivity_check.rdma', 'inter_group_wave_pairs', None),
        )
    ...
    def _get_peer_failure_threshold(self):
        """
        Fraction in (0, 1]. Nodes with ``failed_peers / (n-1) >= threshold`` are pruned before inter-group.

        Config: ``prune_failure_threshold`` (default ``0.5``). Invalid or out-of-range values fall back to 0.5.
        """
        cfg = self.config_dict or {}
        raw = get_nested_config(cfg, 'connectivity_check.rdma', 'prune_failure_threshold', 0.5)
        try:
            t = float(raw)
        except (TypeError, ValueError):
            log.warning(f"Invalid prune_failure_threshold {raw!r}; using 0.5")
            t = 0.5
        if t <= 0 or t > 1:
            log.warning(f"prune_failure_threshold {t} must be in (0, 1]; using 0.5")
            t = 0.5
        return t
    ...
    def _apply_intra_prune(self, intra_results, groups):
        ...
        Config:
            ``prune_failure_threshold`` (default ``0.5``): prune when fraction >= threshold.

        Returns:
            tuple: (pruned_node_set, pruned_records, groups_for_inter)
        """
        threshold = self._get_peer_failure_threshold()
        log.info(
            f"Round 1 pruning: peer-failure fraction threshold {threshold:.0%} "
            f"(prune_failure_threshold); prune when fraction ≥ threshold per node."
        )
                    ...
                    pruned_records.append(
                        {
                            ...
                            'reason': (
                                ...
                                f'≥ threshold {threshold:.0%} (prune_failure_threshold).'
                            ),
                        }
                    )
    ...
    def _execute_intragroup(self, groups, port_start):
        ...
        return self._execute_test_round_with_retries(groups, "intra_group", port_start, work_segment='intra_group')

    def _generate_all_intergroup_pairs(self, groups):
        """
    ...
    def _execute_test_round_with_retries(self, test_groups, round_type, port_start, *, work_segment='round'):
        """
        Execute testing round with script coordination using class attributes.

        Retries subsets that fail with ``PORT_LISTEN_FAILED`` up to ``port_retry_max`` times
        ...
        Args:
            ...
            work_segment: Path segment for this round's remote work dir (under
                ``artifacts_root_dir/rdma_connectivity_workspace/<session>/``).
        ...
        while pending:
            ...
            batch = self._execute_test_assignments(pending, round_type, seg)
            merged.update(batch)

            pl_failed_keys = {
                pk for pk, res in batch.items() if isinstance(res, dict) and self._is_port_listen_failure(res)
            }
            ...
            pk_to_assignment = {self._generate_test_pair_key(a): a for a in pending}
            pending = []
    ...
    def _execute_test_assignments(self, assignments, round_type, work_segment):
        """
        Execute a complete RDMA connectivity test round for the given assignments.

        Orchestrates a three-phase testing cycle:
        1. Server Phase: Deploy and start ibv_rc_pingpong servers on assigned nodes
        2. Client Phase: Deploy and execute ibv_rc_pingpong clients to connect to servers
        3. Collection Phase: Gather and analyze test results from server/client logs

        Args:
            assignments: List of test assignments, each containing server_node, client_node,
                        server_iface, client_iface, and port
            round_type: Type of round being executed ("intra_group", "inter_group", etc.)
            work_segment: Path segment for remote workspace directory

        Returns:
            dict: Test results mapped by test pair keys
        """
        from cvs.lib.scriptlet import ScriptLet

        scriptlet_debug = self._scriptlet_enabled()
        workspace_dir = self._artifact_workspace_dir(work_segment)
        ...
        with ScriptLet(
            ...
            for host in self.phdl.reachable_hosts:
                ...
                if script_content:
                    ...
                    server_scriptlet.create_script(script_id, script_content)
            ...
            if server_script_mapping:
                server_scriptlet.copy_script_list(server_script_mapping)
            ...
            with ScriptLet(
                ...
                for host in self.phdl.reachable_hosts:
                    ...
                    if script_content:
                        ...
                        client_scriptlet.create_script(script_id, script_content)
                ...
                if client_script_mapping:
                    client_scriptlet.copy_script_list(client_script_mapping)

                    if server_script_mapping:
                        log.info(f"Starting {len(server_script_mapping)} server scripts in parallel")
                        server_results = server_scriptlet.run_parallel_group(server_script_mapping, timeout=90)
                    ...
                    client_execution_results = client_scriptlet.run_parallel_group(
                        client_script_mapping, timeout=self.timeout
                    )
                ...
                # Client context exits here; server context remains active until this block completes.
        # Server context exits here (after client phase) and runs its cleanup.

        # Phase 3: Collection and analysis (keeping existing separate context)
            ...
            test_metadata.append(
                {
                    ...
                    'port': port,
                    'client_log_path': (
                        f"{workspace_dir}/client_{client_iface}_{port}_{server_node.replace('.', '_').replace(':', '_')}.log"
                    ),
                    'server_log_path': (
                        f"{workspace_dir}/server_{server_iface}_{port}_{client_node.replace('.', '_').replace(':', '_')}.log"
                    ),
                }
            )

        return self._collect_results_with_scriptlet(test_metadata, workspace_dir)

    def _generate_server_commands_for_host(self, host, assignments, workspace_dir):
        commands = []
        workspace_dir = workspace_dir
        for assignment in assignments:
            if assignment['server_node'] == host:
                ...
                client_node = assignment['client_node']
                port = assignment['port']
                client_node_token = client_node.replace('.', '_').replace(':', '_')
                srv_log = f"{workspace_dir}/server_{server_iface}_{port}_{client_node_token}.log"
                if self._scriptlet_enabled():
                    trace_log = f"{workspace_dir}/strace_server_{server_iface}_{port}_{client_node_token}.log"
                    cmd = (
    ...
    def _generate_client_commands_for_host(self, host, assignments, workspace_dir):
        commands = []
        workspace_dir = workspace_dir
        for assignment in assignments:
            if assignment['client_node'] == host:
                ...
                server_node = assignment['server_node']
                port = assignment['port']
                server_node_token = server_node.replace('.', '_').replace(':', '_')
                cmd = (
                    f"timeout 30 ibv_rc_pingpong -d {client_iface} -g {self.gid_index} -p {port} "
                    f"{server_node} > {workspace_dir}/client_{client_iface}_{port}_{server_node_token}.log 2>&1 &"
                )
                commands.append(cmd)
        return commands

    def _generate_ibv_server_commands(self, host, test_groups, round_type, port_start, workspace_dir="/tmp/preflight"):
        """
        ...
        Args:
            ...
            round_type: "intra_group" or "inter_group"
            port_start: Starting port number
            workspace_dir: Temporary directory for logs

        Returns:
            list: Server commands for this host
        """
        assignments = self._calculate_port_assignments(test_groups, round_type, port_start)
        return self._generate_server_commands_for_host(host, assignments, workspace_dir)

    def _generate_ibv_client_commands(self, host, test_groups, round_type, port_start, workspace_dir="/tmp/preflight"):
        """
        ...
        """
        assignments = self._calculate_port_assignments(test_groups, round_type, port_start)
        return self._generate_client_commands_for_host(host, assignments, workspace_dir)

    def _format_ibv_commands_for_display(self, server_iface, client_iface, server_node, port):
        """
    ...
    def _create_collection_script(self, test_metadata_for_node, workspace_dir):
        """
        Create ibv result collection script.

        Args:
            test_metadata_for_node: Test metadata for a specific node
            workspace_dir: Remote directory where client/server logs for this round live
        ...
        script_lines = [
            ...
            "    shopt -s nocasematch",
            "    if [[ \"$content\" =~ [0-9]+[[:space:]]+bytes[[:space:]]+in[[:space:]]+.*[[:space:]]+seconds ]] || [[ \"$content\" =~ local[[:space:]]+address:.*GID.*remote[[:space:]]+address:.*GID ]]; then",
            ...
            "    if [[ \"$content\" =~ Failed[[:space:]]+to[[:space:]]+modify[[:space:]]+QP.*to[[:space:]]+RTR ]]; then",
            "        echo \"${test_key}=QP_RTR_FAILED\"",
            "    elif [[ \"$content\" =~ Failed[[:space:]]+to[[:space:]]+modify[[:space:]]+QP.*to[[:space:]]+RTS ]]; then",
            "        echo \"${test_key}=QP_RTS_FAILED\"",
            "    elif [[ \"$content\" =~ Couldn.*t[[:space:]]+connect[[:space:]]+to|Unable[[:space:]]+to[[:space:]]+Connect ]]; then",
            "        echo \"${test_key}=CONNECTION_FAILED\"",
            "    elif [[ \"$content\" =~ Failed[[:space:]]+status[[:space:]]+transport[[:space:]]+retry[[:space:]]+counter[[:space:]]+exceeded ]]; then",
            "        echo \"${test_key}=TRANSPORT_RETRY_EXCEEDED\"",
            "    elif [[ \"$content\" =~ parse[[:space:]]+WC[[:space:]]+failed ]]; then",
            "        echo \"${test_key}=WC_PARSE_FAILED\"",
            "    elif [[ \"$content\" =~ No[[:space:]]+space[[:space:]]+left[[:space:]]+on[[:space:]]+device ]]; then",
            "        echo \"${test_key}=NO_SPACE_LEFT\"",
            "    elif [[ \"$content\" =~ Couldn.*t[[:space:]]+listen|listen[[:space:]]+to[[:space:]]+port|Address[[:space:]]+already[[:space:]]+in[[:space:]]+use|bind:[[:space:]]+Address[[:space:]]+already[[:space:]]+in[[:space:]]+use ]]; then",
        ...
        script_lines.extend(
            [
                "",
                f"# Logs and artifacts remain under {workspace_dir} on each node",
                "",
                "exit 0",
            ]
        )
    ...
    def _collect_results_with_scriptlet(self, test_metadata, workspace_dir):
        """
        Collect test results using scriptlet with class attributes.

        Args:
            test_metadata: Test metadata list
            workspace_dir: Remote directory for this coordination round (same as server/client logs)
    ...
    def _build_server_script(self, host, server_commands):
        """Build server startup script for ibv_rc_pingpong processes."""
...
"""
    ...
    def _build_client_script(self, host, client_commands):
        """Build client execution script for ibv_rc_pingpong processes."""
