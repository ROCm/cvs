...
# RDMA connectivity testing using ScriptLet infrastructure for unified execution
...
class RdmaConnectivityCheck(PreflightCheck):
    ...
    def _report_output_dir(self) -> str:
        cfg = self.config_dict or {}
        return (get_nested_config(cfg, 'reporting', 'report_output_dir', '/tmp/preflight_reports')).rstrip('/')
    ...
    def _execute_basic(self, port_start):
        ...
        assignments = self._convert_pairs_to_scriptlet_assignments(pairs, port_start)

        # Execute via ScriptLet infrastructure
        log.info(f"Executing {len(assignments)} test assignments via ScriptLet")
        results = self._execute_assignments_scriptlet_round(assignments, f"{self.mode}_mode", f"{self.mode}_single")

        log.info(f"Completed {self.mode} mode testing: {len(results)} results")
        return results

    def _convert_pairs_to_scriptlet_assignments(self, pairs, base_port):
        """
    ...
    def _execute_intergroup_waves(self, groups_inter, port_start, cfg):
        ...
        Config (``cfg``):
            ...
            inter_group_wave_pairs: max ordered group-pairs (Gi→Gj keys) per wave when
        ...
        raw = get_nested_config(cfg, 'connectivity_check.rdma', 'inter_group_wave_pairs', None)
