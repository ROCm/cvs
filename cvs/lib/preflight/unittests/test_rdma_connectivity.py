...
class TestConnectivityMathInvariant(unittest.TestCase):
    ...
    @patch.object(RdmaConnectivityCheck, '_execute_test_round_with_retries')
...
class TestIntraPeerPrune(unittest.TestCase):
    ...
    def test_prunes_nodes_at_half_threshold_three_node_group(self):
        ...
        checker = _make_checker(config_dict={'connectivity_check': {'rdma': {'prune_failure_threshold': 0.5}}})
    ...
    def test_no_prune_when_below_threshold(self):
        ...
        checker = _make_checker(config_dict={'connectivity_check': {'rdma': {'prune_failure_threshold': 0.6}}})
...
class TestGenerateServerCommandsScriptletDebug(unittest.TestCase):
    """Server scriptlet lines: optional strace when scriptlet is set."""

    def test_default_no_strace(self):
        checker = _make_checker(expected_interfaces=['if1'], config_dict={})
        groups = {'group_1': ['A', 'B']}
        cmds = checker._generate_ibv_server_commands('A', groups, 'intra_group', 5000, '/tmp/preflight')
        self.assertTrue(cmds)
        self.assertTrue(all('strace' not in c for c in cmds))

    def test_scriptlet_debug_adds_strace(self):
        checker = _make_checker(expected_interfaces=['if1'], config_dict={'debug': {'scriptlet': True}})
        groups = {'group_1': ['A', 'B']}
        cmds = checker._generate_ibv_server_commands('A', groups, 'intra_group', 5000, '/tmp/preflight')
        self.assertTrue(cmds)
        for c in cmds:
