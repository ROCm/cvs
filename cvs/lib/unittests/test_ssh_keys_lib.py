...
class TestValidateKeyDistributionConfig(unittest.TestCase):
    ...
    def test_valid_config_returns_normalized(self, _isfile):
        ...
        self.assertEqual(norm["ssh_dir_nfs_shared"], False)
...
class TestExpandRemoteDir(unittest.TestCase):
    def test_absolute_path_returned_unchanged(self):
        orch = MagicMock()
        result = lib._expand_remote_dir(orch, "/home/user/.ssh")
        self.assertEqual(result, "/home/user/.ssh")
        orch.exec.assert_not_called()

    def test_tilde_path_expanded_via_exec(self):
        orch = MagicMock()
        orch.exec.return_value = {"n1": "/home/user/.ssh\n"}
        result = lib._expand_remote_dir(orch, "~/.ssh")
        self.assertEqual(result, "/home/user/.ssh")
        orch.exec.assert_called_once_with("echo ~/.ssh", timeout=10)
...
class TestUploadClusterKeys(unittest.TestCase):
    ...
    def test_upload_called_for_priv_and_pub(self, _isfile):
        ...
        self.assertIn("/home/user/.ssh/cluster_id", remote_paths)
        self.assertIn("/home/user/.ssh/cluster_id.pub", remote_paths)
    ...
    @patch("cvs.lib.ssh_keys_lib.os.path.isfile", return_value=True)
    def test_nfs_shared_uploads_via_head_only(self, _isfile):
        orch = self._make_orch()
        norm = {
            "cluster_key_private_path": "/local/id",
            "cluster_key_public_path": "/local/id.pub",
        ...
        results = lib.upload_cluster_keys(orch, norm)
        self.assertEqual(orch.head.upload_file.call_count, 2)
        orch.all.upload_file.assert_not_called()
        self.assertTrue(all(results.values()))

    def test_nfs_shared_head_upload_ioerror_marks_all_nodes_failed(self):
        orch = self._make_orch()
        orch.head.upload_file.side_effect = IOError("sftp fail")
        norm = {
            "cluster_key_private_path": "/local/id",
            "cluster_key_public_path": "/local/id.pub",
            "key_name": "cluster_id",
...
class TestAuthorizeControllingStation(unittest.TestCase):
    ...
    def test_upload_and_authorize_called(self):
        ...
        def exec_side_effect(cmd, timeout=30, detailed=False):
            if cmd.startswith("echo "):
                return {"n1": "/home/user/.ssh"}
            return {"n1": {"output": "", "exit_code": 0}}
        ...
        results = lib.authorize_controlling_station(orch, norm)
        orch.all.upload_file.assert_called_once_with("/local/ctrl.pub", "/home/user/.ssh/.cvs_controlling_station.pub")
        self.assertTrue(results["n1"])
    ...
    def test_nfs_shared_uploads_via_head_only(self):
        orch = MagicMock()
        orch.all.reachable_hosts = ["n1", "n2"]
