...
class GateMatrixConfig:
    """Configuration for gate matrix rendering."""

    tier_order: tuple[str, ...] = ()


class GateMatrixRenderer:
    """Build and render gate matrix rows, tables, and heatmaps."""

    def __init__(self, config: GateMatrixConfig | None = None):
        self.config = config or GateMatrixConfig()
    ...
    def build_rows(self, cells: List[dict]) -> List[dict]:
        return [
            {
...
"""
    ...
    def render_table(self, gate_matrix: List[dict], tier_order: Iterable[str]) -> str:
        tiers = tuple(tier_order) or self.config.tier_order
        if not gate_matrix:
    ...
    def render_heatmap(self, gate_matrix: List[dict], tier_order: Iterable[str]) -> str:
        tiers = tuple(tier_order) or self.config.tier_order
...
def gate_cell_label(cell: Mapping[str, object]) -> str:
    return GateMatrixRenderer.cell_label(cell)


def build_gate_matrix_rows(cells: List[dict]) -> List[dict]:
    return GateMatrixRenderer().build_rows(cells)


def gate_matrix_table_css() -> str:
    return GateMatrixRenderer.table_css()


def render_gate_matrix_html(gate_matrix: List[dict], tier_order: Iterable[str]) -> str:
    return GateMatrixRenderer().render_table(gate_matrix, tier_order)


def render_gate_heatmap_html(gate_matrix: List[dict], tier_order: Iterable[str]) -> str:
    return GateMatrixRenderer().render_heatmap(gate_matrix, tier_order)


def gate_heatmap_css() -> str:
    return GateMatrixRenderer.heatmap_css()
