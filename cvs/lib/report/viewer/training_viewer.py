...
Interactive viewer for the training Run Deck (framework-generic: jaxmaxtext,
megatron, torchtitan, ...).

Unlike the inference viewer (concurrency/ISL sweeps), the training viewer plots
per-tag line charts (all sweeps overlaid = cross-sweep comparison) DYNAMICALLY
with Chart.js from the embedded JSON payload -- the raw TensorBoard series
collected per sweep. No PNGs. A default set of series is shown, the rest are
multi-selectable, initial steps can be skipped, and charts support zoom/pan.
'''
...
def write_training_viewer(
    ...
    *,
    ...
    doc = (
        template.replace("__TITLE__", html.escape(title or "Training Run Deck"))
