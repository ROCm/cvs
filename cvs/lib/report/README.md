...
| **CI summary** | Inference suite with `--html` | `{report_basename}_summary.html` | One-page status: overall badge, worst cells, baseline regressions, link to full report |
...
├── panels/prev_run.py            # optional run-to-run delta panel
...
| `panels` | Optional `prev_run`, … |
| `summary` | Truncation mode and viewer basename when cell count is large |

CI summary is a separate file (`{basename}_summary.html`).
