...
class ProfileConfigResolver:
    ...
    @classmethod
    def import_callable(cls, spec: str) -> Callable[..., Any]:
        obj = cls.import_object(spec)
        if not callable(obj):
    ...
    def from_profile_dict(cls, profile: dict[str, Any]) -> InferenceReportConfig:
        ...
        if builder in ("series", "matrix", "preflight"):
        ...
        tier_metric_specs = (
            cls.import_callable(hooks.get("tier_metric_specs") or sweep.get("tier_metric_specs_hook", ""))
        ...
        results_columns = cls._parse_results_columns(
            sweep.get("results_table_columns") or profile.get("results_columns") or []
        )
        chart_series = cls._parse_chart_series(sweep.get("chart_series") or profile.get("chart_series") or [])
        ...
        if hooks.get("launch_provenance"):
            launch_builder = cls.import_callable(hooks["launch_provenance"])
        ...
        return make_inference_report_config(
            ...
            or profile.get("inference_test_substring"),
