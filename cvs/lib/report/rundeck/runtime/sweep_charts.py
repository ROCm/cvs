...
class SweepChartRenderer:
    """Render static concurrency bar charts and series charts for Run Deck."""

    _ACCENTS = ("accent", "accent2", "accent3")

    @staticmethod
    def _group_keys(chart_cfg: list, chart_series: dict) -> List[Tuple[str, str, str]]:
    ...
    @staticmethod
    def _y_ticks(min_val: float, max_val: float, *, count: int = 5) -> List[float]:
        if count < 2:
            return [max_val]
    ...
    @classmethod
    def _display_scale(cls, min_val: float, max_val: float) -> Tuple[float, float, List[float]]:
        if max_val > min_val:
            domain_min, domain_max = min_val, max_val
        else:
            pad = max(abs(max_val) * 0.08, 1.0)
            domain_min, domain_max = max_val - pad, max_val + pad
        ticks = cls._y_ticks(domain_min, domain_max)
        return domain_min, domain_max, ticks

    @classmethod
    def _value_pct(cls, val: float, domain_min: float, domain_max: float) -> float:
        span = domain_max - domain_min or 1.0
        return max(0.0, min(100.0, 100.0 * (val - domain_min) / span))

    @classmethod
    def _bar_height_pct(cls, val: float, min_val: float, max_val: float) -> float:
        domain_min, domain_max, _ = cls._display_scale(min_val, max_val)
        return max(8.0, cls._value_pct(val, domain_min, domain_max))
    ...
    def render_bar_chart(
        ...
        *,
        ...
        domain_min, domain_max, ticks = self._display_scale(min_val, max_val)
        y_labels = "".join(
            f"<span class='chart-ylbl' style='bottom:{self._value_pct(t, domain_min, domain_max):.2f}%'>"
            f"{html.escape(fmt_num(t))}</span>"
            for t in ticks
        )
        grid = "".join(
            f"<span class='chart-hline' style='bottom:{self._value_pct(t, domain_min, domain_max):.2f}%'></span>"
            for t in ticks
        )
        bars = []
        x_labels = []
        for x_value, val in points:
            ...
            tip = html.escape(f"{x_label}={fmt_num(x_value)}: {fmt_num(val)} {unit}".strip())
            bars.append(
            ...
            x_labels.append(f"<span class='chart-xlbl'>{html.escape(x_label)}={fmt_num(x_value)}</span>")
        return (
    ...
    def render_sweep_charts(self, chart_cfg: list, chart_series: dict) -> str:
        group_keys = self._group_keys(chart_cfg, chart_series)
        chart_sections = []
    ...
    def render_series_chart(self, title: str, points: list, unit: str, x_label="X") -> str:
        normalized = []
        for p in points:
...
def render_bar_chart(
    ...
    *,
    ...
    return _DEFAULT_RENDERER.render_bar_chart(title, points, unit, accent=accent)


def render_sweep_charts_html(chart_cfg: list, chart_series: dict) -> str:
    return _DEFAULT_RENDERER.render_sweep_charts(chart_cfg, chart_series)


def render_series_chart_html(title: str, points: list, unit: str) -> str:
    return _DEFAULT_RENDERER.render_series_chart(title, points, unit)
