...
def validate_key_distribution_config(config_dict):
    ...
    norm.setdefault("ssh_dir_nfs_shared", False)
...
def build_write_ssh_config_cmd(remote_ssh_dir, block_text, mode):
    ...
    sed_script = shlex.quote(f"/{SSH_CONFIG_BEGIN}/,/{SSH_CONFIG_END}/d")
    sed_del = f"sed -i {sed_script} {cfg} 2>/dev/null || true"
...
def upload_cluster_keys(orch, norm_config):
    ...
    When remote_ssh_dir is NFS-shared across nodes (ssh_dir_nfs_shared=True), the
    keypair is uploaded once via the head node only -- every other node already
    sees the same file through the shared mount, so a per-node upload would just
    duplicate the same SFTP transfer.
    """
    ...
    sftp_dir = _expand_remote_dir(orch, remote_ssh_dir)
    remote_priv = f"{sftp_dir}/{key_name}"
    remote_pub = f"{sftp_dir}/{key_name}.pub"

    results = {node: True for node in orch.all.reachable_hosts}
    upload_target = orch.head if nfs_shared else orch.all

    for local, remote in ((priv_local, remote_priv), (pub_local, remote_pub)):
        try:
            upload_target.upload_file(local, remote)
        except IOError as e:
...
def authorize_controlling_station(orch, norm_config):
    ...
    nfs_shared = norm_config.get("ssh_dir_nfs_shared", False)
    sftp_dir = _expand_remote_dir(orch, remote_ssh_dir)
    remote_tmp = f"{sftp_dir}/.cvs_controlling_station.pub"

    results = {node: True for node in orch.all.reachable_hosts}
    upload_target = orch.head if nfs_shared else orch.all
    try:
        upload_target.upload_file(controlling_local, remote_tmp)
    except IOError as e:
...
def _expand_remote_dir(orch, remote_dir):
    """Resolve a leading '~' to an absolute path for SFTP use.

    orch.exec runs through a remote shell (which expands '~'), but SFTP
    transfers (upload_file/copy_file) go over a raw SFTP channel with no
    shell involved -- a literal '~' is treated as an ordinary path
    component, silently writing into a directory named '~' instead of
    the real home directory.
    """
    if not remote_dir.startswith("~"):
        return remote_dir
    out = orch.exec(f"echo {remote_dir}", timeout=10)
    return next(iter(out.values())).strip()
