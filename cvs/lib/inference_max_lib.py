...
class InferenceMaxJob():
    ...
    def __init__( self,  c_phdl, s_phdl, model_name, 
                 inference_config_dict, benchmark_params_dict,
                 hf_token, gpu_type='mi300', distributed_inference=False ):
    ...
    def run_preinference_tasks(self, ):
    ...
    def launch_docker_container(self, container_name, image, device_list, volume_dict, env_dict ):
        if self.distributed_inference is True:
            docker_lib.launch_docker_container( self.s_phdl, container_name, image, 
    ...
    def exec_nic_setup_scripts( self, ):
        ...
        if self.distributed_inference is True:
            ...
            if re.search( 'broadcom|thor', self.nic_type, re.I ):
                ...
                out_dict = self.s_phdl.exec(f'docker exec {self.container_name} /bin/bash -c "sudo \
    ...
    def build_server_inference_job_cmd( self, ):
    ...
    def start_inference_client_job( self, ):
    ...
    def poll_for_inference_completion( self, waittime_between_iters=60,  \
            ...
            for metric_name in self.expected_result_dict[node].keys():
                if metric_name in expected_result_dict:
                    ...
                    if re.search( 'ms', metric_name, re.I ):
                        if float(self.inference_result_dict[node][metric_name]) > \
                            float(self.expected_result_dict[metric_name]):
                            fail_test(f"FAIL - The latency metric {metric_name} actual value higher than expected \
                                Actual = {self.inference_result_dict[node][metric_name]},  \
                                Expected = {self.expected_result_dict[metric_name]}")
                    else:
                        if float(self.inference_result_dict[node][metric_name]) < \
                            float(self.expected_result_dict[metric_name]):
                            fail_test(f"FAIL - The latency metric {metric_name} actual value lower than expected \
                                Actual = {self.inference_result_dict[node][metric_name]}, \
                                Expected = {self.expected_result_dict[metric_name]}")
