...
def check_perftest_dmabuf_support(phdl, binary_path):
    ...
    Returns:
        bool: True if the binary supports --use_rocm_dmabuf on any host,
              False otherwise.
    """
    if binary_path in _dmabuf_support_cache:
        log.debug('Using cached dmabuf result for %s: %s', binary_path, _dmabuf_support_cache[binary_path])
        return _dmabuf_support_cache[binary_path]
    ...
    try:
        ...
        for host, result in host_results.items():
            ...
            if "use_rocm_dmabuf" in output:
                ...
                supported_hosts.append(host)
            else:
                log.debug("Host %s: binary %s does not support --use_rocm_dmabuf", host, binary_path)

        dmabuf_supported_any = bool(supported_hosts)
        log.info(
            "dmabuf support for %s: %s (%d/%d hosts)",
            binary_path,
            "yes" if dmabuf_supported_any else "no",
            len(supported_hosts),
            len(host_results),
        )
        _dmabuf_support_cache[binary_path] = dmabuf_supported_any
