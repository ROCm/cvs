...
Client load generation prefers the **vLLM install’s** ``benchmarks/<script>`` when present, then
``/app/bench_serving/<script>`` (cloned at runtime for legacy ROCm images), then
``python -m vllm.entrypoints.cli.main bench serve`` when the CLI exposes it.
'''
...
def bash_export_bench_script_from_vllm_install(bench_serv_script: str) -> str:
    ...
    ``site-packages/benchmarks/``, ``/app/bench_serving/``) because some vLLM wheels omit
    ``benchmarks/`` unless built with bench extras; the probe opens the file before accepting a path.

    If no script file exists but the install exposes the vLLM CLI bench entry
    ...
    py_body = textwrap.dedent(
        ...
        for sp in site.getsitepackages():
            ...
            add(sp / "benchmarks" / n)
        add(pathlib.Path("/app/bench_serving") / n)
        chosen = None
        for c in cands:
        ...
        if importlib.util.find_spec("vllm.entrypoints.cli.main") is None:
            sys.stderr.write(
                ...
                "or configure benchmark_script_repo for bench_serving clone.\\n" % (n,)
            )
            sys.exit(1)
        try:
            r = subprocess.run(
        ...
        if r is None or r.returncode != 0:
            sys.stderr.write(
                ...
                "Many wheels omit ``vllm/benchmarks/``; install ``vllm[bench]``, clone "
                "bench_serving (benchmark_script_repo), or match your Python interpreter "
                "to the vLLM install.\\n" % (n,)
            )
            sys.exit(1)
        print("export BENCH_PY=" + esc(sys.executable))
