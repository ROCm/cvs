...
export default function TileLauncher() {
  const { tilesBlocked } = useOutletContext<{ tilesBlocked: boolean }>();
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {TILES.map((t) =>
          tilesBlocked ? (
            <div
              key={t.id}
              className="rounded-xl border border-border bg-card p-6 shadow-sm opacity-40 cursor-not-allowed"
              title="Fleet probe in progress — available shortly"
            >
              <div className="mb-4 inline-flex rounded-lg bg-primary/10 p-3 text-primary">
                <t.icon className="h-6 w-6" />
              </div>
              <h2 className="mb-1 text-lg font-semibold">{t.name}</h2>
