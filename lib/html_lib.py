             ...
             for eth_device in d_dict[node].keys():
                 ...
                 if d_dict[node][eth_device]:
                     ...
                     if non_zero_diff:
                         ...
                         for stats_key in stats_dict.keys():
                             if int(stats_dict[stats_key]['diff']) > 0:
                                 fp.write(f"<tr><td>{stats_key}</td><td>{stats_dict[stats_key]['before']}</td><td>{stats_dict[stats_key]['after']}</td><td><span class=\"label label-danger\">{stats_dict[stats_key]['diff']}</span></td></tr>\n")
                         fp.write(f'</table></td>\n')
                     else:
                         fp.write('<td></td>')
                 else:
