...
class TestPsshExec(unittest.TestCase):
    ...
    def test_exec_with_exception_stop_on_errors_false(self, mock_check_connectivity):
        ...
        self.mock_client.run_command.return_value = [mock_output1, mock_output2]
        mock_check_connectivity.return_value = True  # No pruning
    ...
    def test_exec_with_pruning_unreachable_host(self, mock_check_connectivity, mock_pssh_client):
        # Test: With stop_on_errors=False, timeout on host2, and check_connectivity fails for host2, prune it
        self.mock_client = MagicMock()
        mock_pssh_client.return_value = self.mock_client
        self.host_list = ['host1', 'host2']
        self.pssh = Pssh('log', self.host_list, user='user', password='pass')
        ...
        self.mock_client.run_command.return_value = [mock_output1, mock_output2]
        mock_check_connectivity.return_value = False  # Simulate unreachable
        ...
        self.assertIn('host2', result)
        self.assertIn('success output', result['host1'])
        self.assertEqual(result['host2'], 'Host Unreachable')
    ...
    def test_exec_pruning_with_multiple_unreachable_hosts(self, mock_check_connectivity, mock_pssh_client):
        ...
        mock_output3.stdout = []
        mock_output3.stderr = []
        mock_output3.exception = Timeout('Command timed out')

        self.mock_client.run_command.return_value = [mock_output1, mock_output2, mock_output3]
        mock_check_connectivity.return_value = False  # Simulate all unreachable
        ...
        self.assertIn('success output', result['host1'])
        self.assertEqual(result['host2'], 'Host Unreachable')
        self.assertEqual(result['host3'], 'Host Unreachable')
        # Client should be recreated once (init + prune)
        self.assertEqual(mock_pssh_client.call_count, 2)

    @patch('lib.parallel_ssh_lib.ParallelSSHClient')
    def test_exec_no_pruning_on_non_timeout_exception(self, mock_pssh_client):
        # Test: With stop_on_errors=False, non-Timeout exception on host2, no pruning occurs
        # Non-Timeout exceptions are treated as transient failures (e.g., authentication or network glitches),
        # not indicative of permanent unreachability, so we don't attempt connectivity checks or pruning
        self.mock_client = MagicMock()
        mock_pssh_client.return_value = self.mock_client
        self.host_list = ['host1', 'host2']
        self.pssh = Pssh('log', self.host_list, user='user', password='pass')
    ...
    def test_exec_no_pruning_when_stop_on_errors_true(self, mock_inform, mock_prune):
        # Test: With stop_on_errors=True, no pruning even with timeout
...
class TestPsshExecCmdList(unittest.TestCase):
    ...
    @patch.object(Pssh, 'check_connectivity')
    def test_exec_cmd_list_with_exception_stop_on_errors_false(self, mock_check_connectivity):
        ...
        self.mock_client.run_command.return_value = [mock_output1, mock_output2]
        mock_check_connectivity.return_value = True  # Simulate reachable, no pruning
    ...
    @patch('lib.parallel_ssh_lib.ParallelSSHClient')
    def test_exec_cmd_list_no_pruning_on_non_timeout_exception(self, mock_pssh_client):
        # Test: exec_cmd_list with non-Timeout exception, no pruning occurs
        # Non-Timeout exceptions are treated as transient failures, so no connectivity checks or pruning
        self.mock_client = MagicMock()
        mock_pssh_client.return_value = self.mock_client
        self.host_list = ['host1', 'host2']
        self.pssh = Pssh('log', self.host_list, user='user', password='pass')
    ...
    def test_exec_cmd_list_no_pruning_when_stop_on_errors_true(self, mock_inform, mock_prune):
        # Test: exec_cmd_list with stop_on_errors=True, no pruning even with timeout
        # Since stop_on_errors=True, run_command raises immediately, so prune_unreachable_hosts and inform_unreachability are not invoked
        cmd_list = ['echo test']
        from pssh.exceptions import Timeout
        self.mock_client.run_command.side_effect = Timeout('Command timed out')
