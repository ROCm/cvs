...
type Field struct {
	...
	Example     string  `json:"example,omitempty"` // sample value, from a sibling `_example_<field>` key
...
// inferObjectFields turns a map into sorted fields, folding metadata sibling
// keys into the corresponding `<field>`: `_comment_<field>` becomes its
// description and `_example_<field>` becomes its example hint. All `_comment*`
// and `_example*` keys are dropped from the rendered form.
func inferObjectFields(obj map[string]any) []Field {
	...
	for k, val := range obj {
		...
		} else if strings.HasPrefix(k, "_example_") {
			examples[strings.TrimPrefix(k, "_example_")] = exampleString(val)
		}
	}

	fields := []Field{}
	for k, val := range obj {
		if strings.HasPrefix(k, "_comment") || strings.HasPrefix(k, "_example") {
			continue
		}
		f := inferField(k, val)
...
// exampleString renders an `_example_*` value as display text: strings are used
// as-is; anything else is JSON-encoded so structured examples still show.
func exampleString(val any) string {
	if s, ok := val.(string); ok {
		return s
	}
