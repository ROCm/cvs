...
//
// Multi-config directories follow the convention
// `category/framework/{platform}_{framework}_{model}_{topology}.ext`, which we
// parse into facets so the UI can offer a faceted picker (filter, not
// construct). Flat configs (e.g. rccl/rccl_config.json) leave the facet fields
// empty and collapse to a single choice.
type ConfigRef struct {
	...
	Category  string `json:"category,omitempty"`  // top-level dir, e.g. "training"
	Framework string `json:"framework,omitempty"` // framework subdir, e.g. "jax"
	Platform  string `json:"platform,omitempty"`  // e.g. "mi300x"
	Model     string `json:"model,omitempty"`     // e.g. "llama3_1_405b"
	Topology  string `json:"topology,omitempty"`  // "single" | "distributed"
}
...
func (c *ConfigService) Examples(suite Suite) ([]ConfigRef, error) {
	...
	for _, e := range entries {
		...
		refs = append(refs, newConfigRef(rel, format))
	}
