...
func TestHandler_DeleteInventory(t *testing.T) {
	dir := t.TempDir()
	st, err := NewFileStore(filepath.Join(dir, "inventory.json"))
	if err != nil {
		t.Fatalf("NewFileStore: %v", err)
	}
	h := NewHandler(st, nil, nil, nil)
	srv := mountedRouter(h)

	saveBody := `{"nodes":["10.0.0.1"],"username":"u","auth_method":"key","key_name":"k"}`
	req := httptest.NewRequest(http.MethodPost, "/api/v1/inventory", strings.NewReader(saveBody))
	...
	del := httptest.NewRequest(http.MethodDelete, "/api/v1/inventory", nil)
	rec2 := httptest.NewRecorder()
	srv.ServeHTTP(rec2, del)
