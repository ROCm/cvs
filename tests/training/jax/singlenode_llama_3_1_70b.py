...
def test_llama_3_1_70b_singlenode_training(phdl, training_dict, model_params_dict, hf_token ):
    """
    Pytest: End-to-end JAX training test for Llama 3.1 70B.
    The input json file params are set to run the training on single node but in parallel
    ...
    Args:
      ...
      model_params_dict (dict): Model/hyperparameter presets (multi-node, per GPU type).
    ...
    Behavior:
      ...
      - Creates a JaxTrainingJob for model 'llama3.1-70b' without parameter tuning overrides.
    ...
    jx_obj = jax_training_lib.JaxTrainingJob( phdl,
           ...
           hf_token, gpu_type, distributed_training=False, tune_model_params=False )
