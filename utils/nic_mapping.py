...
def print_table(rows: List[Dict]) -> None:
    if not rows:
        ...
        return
    headers = [
        "NIC (BDF)", "NIC vendor/device", "NIC drv/numa",
    ...
    for r in rows:
        ...
        print(f'{r["nic_addr"]:>12} | {nic_meta:<17} | {nic_drv:<12} | '
              f'{r["gpu_addr"]:>12} | {gpu_meta:<17} | {gpu_drv:<12} | '
              f'{(r["lca"] or "-"):>12} | {hops} | {group_hops}')

def write_csv(rows: List[Dict], path: str) -> None:
    fields = [
        "nic_addr","nic_name","nic_vendor","nic_device","nic_driver","nic_numa","is_vf","pf_addr",
...
def main():
    ...
    if args.csv:
        ...
        print("\n# Compact:")
        for r in rows:
            print(f'{r["nic_addr"]} -> {r["gpu_addr"]} | NUMA {r["nic_numa"]}->{r["gpu_numa"]} '
